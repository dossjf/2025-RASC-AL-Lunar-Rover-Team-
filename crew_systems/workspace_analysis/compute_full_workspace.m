...
% --------------------- %
% --------------------- %
function compute_full_workspace(robot_name, robot_config, dh_table, dh_type, num_joints, joint_limits, iter_per_joint)
    addpath("./kinematics/")
    ...
    if(length(robot_config) ~= num_joints)
        error("%s's robot_config[] array must be length of %d!\n", robot_name, num_joints)
    end
    if(isa(robot_config(1), "string"))
    else
